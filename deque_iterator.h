...
namespace mystl
{
    ...
    // 构造函数
    template <typename T>
    deque_iterator<T>::deque_iterator(T *value, map_pointer new_node) : cur(value), first(*new_node), last(first + buffer_size()), node(new_node)
    {
    }

    template <typename T>
    deque_iterator<T>::deque_iterator(const deque_iterator &rhs) : cur(rhs.cur), first(rhs.first), last(rhs.last), node(rhs.node)
    {
    }

    template <typename T>
    deque_iterator<T>::deque_iterator() : cur(nullptr), first(nullptr), last(nullptr), node(nullptr)
    {
    }

    // 重载运算符
    template <typename T>
    typename deque_iterator<T>::difference_type deque_iterator<T>::operator-(const deque_iterator<T> &rhs) const
    {
        return (node - rhs.node) * buffer_size() + (cur - first) - (rhs.cur - rhs.first);
    }
    ...
    template <typename T>
    typename deque_iterator<T>::reference deque_iterator<T>::operator[](difference_type n) const
    {
        return *(*this + n);
    }
