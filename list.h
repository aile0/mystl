...
namespace mystl
{
    ...
    public:
        ...
        using const_reverse_iterator = const mystl::reverse_iterator<const_iterator>;
    ...
    public:
        ...
        list(const std::initializer_list<T> &il) { range_init(il.begin(), il.end()); }
        ...
        void splice(iterator position, list &other_list, iterator first, iterator last);
    ...
    void list<T, Alloc>::transfer(iterator pos, iterator first, iterator last)
    {
        if (pos != last)
        {
            ...
            first.node->prev->next = last.node; // last.node 之前的下一个节点是 last.node
            last.node->prev->next = pos.node;   // first.node 之前的下一个节点是 pos.node
            pos.node->prev->next = first.node;  // pos.node 之前的下一个节点是 first.node

            // 2. 更新 prev 指针
            link_type tmp = pos.node->prev;
            pos.node->prev = last.node->prev;   // pos.node 之前的节点变成 last.node 的前一个节点
            last.node->prev = first.node->prev; // last.node 的前一个节点变成 first.node 的前一个节点
            first.node->prev = tmp;             // first.node 的前一个节点变成原 pos.node 的前一个节点
        }
    }
    ...
    template <typename T, typename Alloc>
    void list<T, Alloc>::splice(iterator position, list &other_list, iterator first, iterator last)
    {
        if (first != last)
