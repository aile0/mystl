...
namespace mystl
{
    ...
    template <typename T>
    void deque<T>::reallocate_map(deque<T>::size_type added_node_size, bool added_front)
    {
        deque<T>::size_type required_map_size = finish.node - start.node + 1 + added_node_size;

        deque<T>::map_pointer new_start;
        if (map_size > 2 * required_map_size)
        {
            new_start = map + (map_size - required_map_size) / 2 + (added_front ? added_node_size : 0);
            if (new_start < start.node)
                std::copy(start.node, finish.node + 1, new_start);
            else
                std::copy_backward(start.node, finish.node + 1, new_start + required_map_size);
        }
        else
        {
            deque<T>::size_type new_map_size = map_size + std::max(map_size, added_node_size) + 2;
            deque<T>::map_pointer new_map = deque<T>::map_alloc::allocate(new_map_size);
            new_start = new_map + (new_map_size - required_map_size) / 2 + (added_front ? added_node_size : 0);
            if (new_start < start.node)
                std::copy(start.node, finish.node + 1, new_start);
            else
                std::copy_backward(start.node, finish.node + 1, new_start + required_map_size);
    ...
    typename deque<T>::iterator deque<T>::expand_front(deque<T>::size_type n)
    {
        ...
        if (left < n)
        {
            ...
            if (required_element % get_map_size() != 0)
                ++required_node;
    ...
    template <typename T>
    void deque<T>::insert_aux(deque<T>::iterator position, deque<T>::size_type n, const deque<T>::value_type &value)
    {
    ...
    template <typename T>
    void deque<T>::fill_init(deque<T>::size_type n, const deque<T>::value_type &value)
    {
        allocate_map(n);
        deque<T>::map_pointer cur;
        try
    ...
    deque<T> &deque<T>::operator=(const deque<T> &rhs)
    {
        if (this != &rhs)
        {
            ...
            {
                deque<T>::const_iterator mid = rhs.begin() + deque<T>::difference_type(len);
    ...
    template <typename T>
    typename deque<T>::const_reference deque<T>::operator[](deque<T>::size_type n) const
    {
        return start[difference_type(n)];
    }
    ...
    template <typename T>
    void deque<T>::insert(deque<T>::iterator position, const deque<T>::value_type &value)
    {
        insert(position, 1, value); // 委托给其他接口，性能不好
    }

    template <typename T>
    void deque<T>::insert(deque<T>::iterator position, deque<T>::size_type n, const deque<T>::value_type &value)
    {
    ...
    template <typename T>
    template <typename InputIterator>
    void deque<T>::insert(deque<T>::iterator position, InputIterator first, InputIterator last)
    {
    ...
    void deque<T>::erase(deque<T>::iterator first, deque<T>::iterator last)
    {
        ...
        else
        {
            ...
            if (elems_before < (size() - n) / 2)
            {
                ...
                for (deque<T>::map_pointer node = start.node; node < new_start.node; ++node)
                {
            ...
            else
            {
                ...
                for (deque<T>::map_pointer node = new_finish.node + 1; node < finish.node; ++node)
                {
                    data_alloc::deallocate(*node, get_map_size());
                }
                finish = new_finish;
    ...
    void deque<T>::clear()
    {
        for (deque<T>::map_pointer node = start.node + 1; node < finish.node; ++node)
        {
            destroy(*node, *node + get_map_size());
            deallocate(*node, get_map_size()); // 如果destroy已经释放内存，这里就不需要了
        }
        if (start.node != finish.node)
        {
            destroy(start.cur, start.last);
            destroy(finish.first, finish.cur);
            deallocate(start.first, get_map_size()); // 如果destroy已经释放内存，这里就不需要了
            deallocate(finish.first, get_map_size());
        }
        else
        {
            destroy(start.cur, finish.cur);
            deallocae(start.first, get_map_size()); // 如果destroy已经释放内存，这里就不需要了
        }
    }
    ...
    template <typename T>
    void deque<T>::resize(deque<T>::size_type new_size, const deque<T>::value_type &value)
    {
        if (new_size < size())
        {
